...
| `showCeilings` | boolean | `true` | Draw the ceiling slabs. Off is often the better floorplan: a ceiling is the one surface you never look at, and in a plan or exploded view it is all you see of the storey below it. In `style: wireframe` the slab stays in the scene as a depth mask, so the storey does not turn into an open box with see-through walls; in `style: solid` it is removed. |
