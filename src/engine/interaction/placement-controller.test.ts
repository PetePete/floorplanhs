...
describe('dragging a marker out of its room', () => {
  it('records the room it came from, so the leader has somewhere to point', () => {
    ...
    // where you can reach it, still saying which room it measures.
    const built = house();
    const entities = {
      moveEntity: () => {},
      setEntities: () => {},
    ...
    // And back inside: the room it landed in, said outright. Letting the
    // position speak for itself reads fine until the marker joins a pile, where
    // the anchor moves to the pile's spot and stops describing the lamp at all.
    placement.beginMove('sensor.a');
